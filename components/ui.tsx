// Shared UI primitives, ported to typed React from UI/refineiq/ui.jsx.
// Presentational only; safe to use in client components.
"use client";

import { useState, type ReactNode, type CSSProperties } from "react";
import { IconDot, IconFile, IconSpark } from "@/components/icons";
...
export const ROLE_TINT_BY_KEY: Record<Role, string> = {
  admin: "#8B5CF6",
  manager: "#3B82F6",
  end_user: "#6B7280",
};

export const DOC_STATUS_TINT: Record<string, string> = {
  Indexed: "#34D399",
  Processing: "#FBBF24",
  Restricted: "#F87171",
  Archived: "#6B7280",
  ...
  return (
    <span
      style={{
        ...
        color: tint, background: `${tint}1f`, border: `1px solid ${tint}3a`, whiteSpace: "nowrap",
      }}
    >
      <IconDot size={size === "sm" ? 5 : 6} color={tint} />
      {role}
    </span>
  );
}

export function StatusChip({ label, tint, soft = true }: { label: string; tint: string; soft?: boolean }) {
  return (
    <span
      style={{
        display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 9px",
        borderRadius: 99, fontSize: 11.5, fontWeight: 550,
        color: tint, background: soft ? `${tint}1c` : tint, border: `1px solid ${tint}38`, whiteSpace: "nowrap",
      }}
    >
      <IconDot size={6} color={tint} />
      {label}
    </span>
  );
}
...
export function Avatar({
  initials, tint = "#6366F1", size = 32, ring = false,
}: { initials: string; tint?: string; size?: number; ring?: boolean }) {
  return (
    <div
      style={{
...
export function Citation({ doc, page, restricted, onRequest }: CitationData & { onRequest?: (doc: string) => void }) {
  const [requested, setRequested] = useState(false);
  return (
    <button
      onClick={() => { if (restricted && !requested) { setRequested(true); onRequest?.(doc); } }}
      className="lift"
      style={{
    ...
    >
      ...
      {restricted && (
        <span style={{ fontSize: 11, fontWeight: 600, marginLeft: 1 }}>
...
export function GlowButton({
  children, onClick, full, disabled, type = "button", size = "md",
}: {
  children: ReactNode;
  onClick?: () => void;
  full?: boolean;
...
/** Tiny inline markdown -> bold/italic + paragraphs + ordered lists. */
export function renderRich(text: string): ReactNode {
  const lines = text.split("\n");
  type Block = { type: "ol"; items: string[] } | { type: "sp" } | { type: "p"; text: string };
  const blocks: Block[] = [];
  ...
  const fmt = (s: string, kp: string): ReactNode[] => {
    const parts = s.split(/(\*\*[^*]+\*\*|\*[^*]+\*)/g).filter(Boolean);
    return parts.map((p, i) => {
      if (p.startsWith("**") && p.endsWith("**")) {
        return <strong key={`${kp}-${i}`} style={{ fontWeight: 650, color: "var(--text)" }}>{p.slice(2, -2)}</strong>;
      }
      if (p.startsWith("*") && p.endsWith("*")) {
        return <em key={`${kp}-${i}`} style={{ color: "var(--a3)" }}>{p.slice(1, -1)}</em>;
      }
      return <span key={`${kp}-${i}`}>{p}</span>;
    });
  };

  const olStyle: CSSProperties = { margin: "4px 0 4px", paddingLeft: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 8 };
        ...
        <ol key={i} style={olStyle}>
          {b.items.map((it, j) => (
            <li key={j} style={{ display: "flex", gap: 11, alignItems: "flex-start" }}>
              <span style={{
                flexShrink: 0, width: 21, height: 21, borderRadius: 6, fontSize: 11, fontWeight: 600,
